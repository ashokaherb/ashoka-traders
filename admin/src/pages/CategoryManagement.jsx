      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-lg shadow-sm divide-y">
          {categories.map((cat) => (
            ...
            >
              {/* min-w-0 lets long names/descriptions wrap instead of widening the row */}
              <div className="flex items-center gap-3 min-w-0">
                {cat.image ? (
                  <img
                    src={getCloudinaryThumbnail(cat.image, 80)}
                    alt=""
                    className="w-10 h-10 rounded object-cover border border-gray-200 shrink-0"
                  />
                ) : (
                  <div className="w-10 h-10 shrink-0 rounded bg-gray-100 flex items-center justify-center text-gray-400 text-sm font-semibold">
                    {cat.name.charAt(0).toUpperCase()}
                  </div>
                )}
                <div className="min-w-0">
                  <p className="font-medium text-gray-800 flex flex-wrap items-center gap-x-2 break-words">
                    {cat.name}
                    {cat.showInNav && (
                  ...
                  {cat.description && <p className="text-sm text-gray-500 break-words">{cat.description}</p>}
                </div>
              </div>
              <div className="flex gap-3 text-sm shrink-0 pl-[52px] sm:pl-0">
